...
export const ChartControls = ({
    ...
    const TIMEFRAME_PRESETS = [
        { label: "Scalp",   intervals: ["1m", "3m", "5m"] },
        { label: "Intraday", intervals: ["5m", "15m", "30m"] },
        { label: "Swing",   intervals: ["1H", "1D", "1W"] },
    ] as const;
    const intervals = ["1m", "3m", "5m", "15m", "30m", "1H", "1D", "1W"];
    ...
    return (
        <div className="flex items-center justify-between bg-background border-b border-border px-3 h-10 select-none w-full z-20">
            ...
            {/* Center-Right: Timeframes + Presets */}
            <div className="flex items-center gap-0.5 px-3 border-l border-r border-white/5 h-full overflow-x-auto no-scrollbar">
                ...
                {TIMEFRAME_PRESETS.map((preset) => (
                    <button
                        key={preset.label}
                        onClick={() => {
                            const next = activePreset === preset.label ? null : preset.label;
                            setActivePreset(next);
                            onIntervalChange(preset.intervals[0]);
                        }}
                        className={cn(
                            "px-2 h-5 flex items-center justify-center text-[8px] font-black rounded-sm tracking-widest uppercase transition-all border mr-1",
                ...
                {/* Individual interval buttons — filtered by active preset if set */}
                {(activePreset
                    ? TIMEFRAME_PRESETS.find(p => p.label === activePreset)?.intervals ?? intervals
                    : intervals
                ).map((int) => (
                    <button
                        key={int}
