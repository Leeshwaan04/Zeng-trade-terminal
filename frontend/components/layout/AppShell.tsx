...
export default function AppShell() {
    ...
    // Apply dynamic height imperatively to avoid inline style lint warning
    useEffect(() => {
        if (accountPanelRef.current) {
            accountPanelRef.current.style.height = `${accountManagerHeight}px`;
        }
    }, [accountManagerHeight]);
    ...
    return (
        <div className="h-screen w-full flex flex-col overflow-hidden selection:bg-primary/30 selection:text-primary relative font-sans">
            ...
            <main className="flex-1 overflow-hidden relative z-0 flex flex-col">
                ...
                {/* Account Manager Bottom Panel — height set imperatively via ref */}
                {isAccountManagerOpen && (
                    <div ref={accountPanelRef} className="w-full shrink-0 z-20 overflow-hidden">
                        <AccountManager />
                    </div>
                )}
            </main>
